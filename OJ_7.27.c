...
void Get_Edge(AGraph* G) {
	...
	if (scanf("%d%c%d", &from, &c, &to) == 0)
		exit(ERROR);
	NodeLink* node1, * node2, *p;
	if (!(node1 = (NodeLink*)malloc(sizeof(NodeLink)))|
		!(node2 = (NodeLink*)malloc(sizeof(NodeLink))))
		exit(ERROR);
	...
	if (G->v[from - 1].first && G->v[from - 1].first->vindex > to) {
		...
		node1->next = G->v[from - 1].first;
		G->v[from - 1].first = node1;
	}

	node2->vindex = from;
	if (G->v[to - 1].first && G->v[to - 1].first->vindex > from) {
		for (p = G->v[to - 1].first; p->next && p->next->vindex > from; p = p->next);
		node2->next = p->next;
		p->next = node2;
	}
	else {
		node2->next = G->v[to - 1].first;
		G->v[to - 1].first = node2;
	}
}
